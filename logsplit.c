...
//      printf("\nEmpty line ignored.");
    ...
    y = lineHeadYear(buf);
    if(y==0) {
      printf("\n%llu: Year not found", reads.lines);
      exit(2);
    }
    if(y != yout) {
      if(fout != NULL) {
  ...
  printf("\nSUCCESSFULLY split %llu lines into %llu files per %llu seconds"
		, reads.lines
		, reads.parts
		, reads.seconds);
...
//		, reads.bytes
//		, (unsigned long long)round(reads.lines / reads.seconds));
  printf("\n");
  return 0;
}
